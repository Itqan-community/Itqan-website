...
export default async function Footer({ locale, }: { locale: string }) {
  ...
  return (
    <footer className="relative bg-primary-900 py-12 sm:py-16 lg:py-20 overflow-hidden mt-auto">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative text-neutral-100">
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-8 lg:gap-0 mb-12 lg:mb-0">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-secondary-100 text-sm sm:text-base font-bold text-center lg:text-start" style={{letterSpacing: '-0.5px'}}>
              {t("contact")}:
            </p>
            <div className="sm:flex sm:flex-col gap-1 items-start">
              <div className="flex gap-8 md:hidden justify-center">
                <Link href="mailto:connect@itqan.dev" className="text-secondary-400 hover:opacity-80 transition-opacity">
                  <FaEnvelope size={20} />
                </Link>
                <Link href="https://github.com/Itqan-community" target="_blank" rel="noopener" className="text-secondary-400 hover:opacity-80 transition-opacity">
                  <FaGithub size={20} />
                </Link>
                <Link href="https://discord.gg/24CskUbuuB" target="_blank" rel="noopener" className="text-secondary-400 hover:opacity-80 transition-opacity">
                  <FaDiscord size={20} />
                </Link>
                <Link href="https://x.com/itqan_community" target="_blank" rel="noopener" className="text-secondary-400 hover:opacity-80 transition-opacity">
                  <FaXTwitter size={20} />
                </Link>
              </div>
              <Link href="mailto:connect@itqan.dev" className="hidden md:flex text-secondary-400 hover:opacity-80 transition-opacity items-center gap-2">
                <FaEnvelope size={20} />
                <span className="text-sm sm:text-base" style={{letterSpacing: '-0.5px'}}>connect@itqan.dev</span>
              </Link>
              <Link href="https://github.com/Itqan-community" target="_blank" rel="noopener" className="hidden md:flex text-secondary-400 hover:opacity-80 transition-opacity items-center gap-2">
                <FaGithub size={20} />
                <span className="text-sm sm:text-base" style={{letterSpacing: '-0.5px'}}>Itqan-community</span>
              </Link>
              <Link href="https://discord.gg/24CskUbuuB" target="_blank" rel="noopener" className="hidden md:flex text-secondary-400 hover:opacity-80 transition-opacity items-center gap-2">
                <FaDiscord size={20} />
                <span className="text-sm sm:text-base" style={{letterSpacing: '-0.5px'}}>Discord</span>
              </Link>
              <Link href="https://x.com/itqan_community" target="_blank" rel="noopener" className="hidden md:flex text-secondary-400 hover:opacity-80 transition-opacity items-center gap-2">
                <FaXTwitter size={20} />
